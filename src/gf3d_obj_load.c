...
{
    slog("gf3d_obj_load_from_file: %s", filename); slog_sync();
    ...
    if (!filename)
    {
        slog("No filename provided to gf3d_obj_load_from_file"); slog_sync();
        return NULL;
    }
    ...
    if (!mem)
    {
        slog("gfc_pak_file_extract returned NULL");
        return NULL;
    }
